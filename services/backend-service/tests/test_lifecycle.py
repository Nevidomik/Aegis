...
async def test_lifespan_reuses_one_client_and_closes_it(
    ...
    monkeypatch.setattr(
        "backend_service.main.create_abuseipdb_http_client", lambda _: client
    )

    async with lifespan(app):
        first = app.state.reputation_provider
        second = app.state.reputation_provider
        assert isinstance(first, AbuseIPDBProvider)
